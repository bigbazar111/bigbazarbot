...
GROUP_ID = -1002256715924  # নির্দিষ্ট গ্রুপ ID
...
async def reply_custom_message(event):
    if event.chat_id == GROUP_ID:  # চেক করবে গ্রুপ ID মিলছে কিনা
...
@client.on(events.NewMessage(pattern=f'^{BOT_PREFIX}setspecialrate\\s+(\\w+)\\s+(\\d+)$'))
async def set_special_rate(event):
    ...
    try:
        ...
        await event.reply(f"➥ Please provide a valid rate. Example: {BOT_PREFIX}setspecialrate 80 70")
    except Exception as e:
        await event.reply(f"❌ Error: {str(e)}")        
# Command: /signup
...
async def show_rates(event):
    ...
    for uc_type, data in uc_stock.items():
        ...
        rates_message += f"☞︎︎︎ {uc_type:<4} 🆄︎🅲︎  ➪  {rate}  Bᴀɴᴋ\n\n"
...
async def check_balance(event):
    ...
    if is_user_signed_up(event.sender_id) or event.sender_id == ADMIN_ID:
        ...
        if target_user_id not in users:
            ...
            return
        balance = users.get(target_user_id, {}).get("balance", 0)
        due = baki_data.get(target_user_id, {}).get("due", 0)
        due_limit = baki_data.get(target_user_id, {}).get("bakiLimit", 0)
        response = (
            "▔▔▔▔▔▔▔▔▔▔▔▔\n"
            f"➪ Nᴀᴍᴇ        : {display_name}\n\n"
            f"➪ Dᴜᴇ          : {due} Tᴋ\n"
            f"➪ Bᴀʟᴀɴᴄᴇ    : {balance} Tᴋ\n"
            f"➪ Dᴜᴇ Lɪᴍɪᴛ : {due_limit} Tᴋ\n"
            "▔▔▔▔▔▔▔▔▔▔▔▔"
        )
        await event.reply(response)
...
async def set_usdt_price(event):
    ...
    try:
        ...
        await event.reply(f"❌ Error: {str(e)}")
# Command: /notifyall
@client.on(events.NewMessage(pattern=f'^{BOT_PREFIX}notifyall(?:\\s+(.+))?$'))
async def notify_all_users(event):
    ...
    try:
        ...
        notified_users = 0
        failed_users = []  # ব্যর্থ ব্যবহারকারীদের তালিকা
        ...
        if event.is_reply:
            ...
            for user_id in users.keys():
                try:
                    ...
                    notified_users += 1
                    print(f"Message forwarded to {user_id}")
                except Exception as e:
            ...
            if not custom_message:
                await event.reply(f"➥ Please provide a message or reply to a message to notify all users.\nExample: `{BOT_PREFIX}notifyall Hello everyone!`")
                return
            ...
            for user_id in users.keys():
                ...
                await asyncio.sleep(1)  # ১ সেকেন্ড বিলম্ব

        # ব্যর্থ ব্যবহারকারীদের তালিকা MongoDB-তে সংরক্ষণ
        notify_message_collection.update_one(
            {"_id": "last_notifyall_failed"},
            {"$set": {"failed_users": failed_users, "timestamp": datetime.now(BD_TIMEZONE).strftime("%Y-%m-%d %H:%M:%S")}},
            upsert=True
        )
        ...
        confirmation = (
            "▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔\n"
            f"☞︎︎︎ Tᴏᴛᴀʟ Usᴇʀs Tᴏ Nᴏᴛɪғʏ ➪ {total_users_to_notify}\n"
            f"☞︎︎︎ Nᴏᴛɪғɪᴄᴀᴛɪᴏɴ Sᴇɴᴛ Tᴏ  ➪ {notified_users}\n"
        )
        if failed_users:
            confirmation += f"☞︎︎︎ Fᴀɪʟᴇᴅ Tᴏ Sᴇɴᴅ Tᴏ ➪ {len(failed_users)}\n" \
                            "Fᴀɪʟᴇᴅ Usᴇʀs:\n" + "\n".join([f"{f['user_id']}: {f['reason']}" for f in failed_users[:5]])
        confirmation += "▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔"
        ...
        if verification.get("status") == "COMPLETED":
            if users[user_id]["balance"] == current_balance:
                ...
                response = (
                    ...
                    f"➪ Yᴏᴜʀ Dᴜᴇ    : {current_due} Tᴋ\n"
                    f"➪ Bᴀʟᴀɴᴄᴇ     : {users[user_id]['balance']} Tᴋ\n"
                    f"➪ Rᴇᴄʜᴀʀɢᴇ Aᴍᴍᴏᴜɴᴛ   : {actual_amount} Tᴋ\n"
                    "➪ Ｐᴀʏᴍᴇɴᴛ ᴍᴇᴛʜᴏᴅ : DrutoPay\n"  # Assuming DrutoPay as the payment method
                    f"➪ Tʀᴀɴsᴀᴄᴛɪᴏɴ ID  : {drutopay_transaction_id}\n"
                    "✧══════✧✧══════✧"
                )
        ...
        async def process_payment():
            try:
                ...
                if result and result.get("transactionStatus") == "Completed":
                    ...
                    total_amount_received = float(result.get("amount", 0))  # ফি-সহ প্রাপ্ত টাকা

                    # ফি হিসেব
                    fee_percentage = 1.15 / 100
                    actual_amount = total_amount_received / (1 + fee_percentage)  # ফি বাদে মূল টাকা
                    fee = total_amount_received - actual_amount
                    actual_amount_rounded = round(actual_amount, 2)
                    fee_rounded = round(fee, 2)

                    # Fetch user entity to get the name
                    user = await client.get_entity(int(user_id))
                    display_name = user.first_name or user.username or "Unknown"
                    ...
                    # Get current balance and due
                    current_balance = users[user_id].get("balance", 0)
                    current_due = baki_data[user_id].get("due", 0)

                    # ফি বাদে শুধু মূল টাকা ব্যালেন্সে যুক্ত করা
                    users[user_id]["balance"] = current_balance + actual_amount_rounded
                    updated_balance = users[user_id]["balance"]
                    updated_due = current_due

                    # Save updated data to MongoDB
                    save_data(users_collection, users)

                    # Update bKash credential limits (ফি-সহ পুরো টাকা)
                    credential = get_current_credential()
                    if credential:
                    ...
                    response = (
                        "🅿🅰🆈🅼🅴🅽🆃 🆁🅴🅲🅸🆅🅴🅳!\n\n"
                        f"➪ Nᴀᴍᴇ        : {display_name}\n"
                        f"➪ Yᴏᴜʀ Dᴜᴇ    : {updated_due} Tᴋ\n"
                        f"➪ Bᴀʟᴀɴᴄᴇ     : {updated_balance} Tᴋ\n"
                        f"➪ Rᴇᴄʹʜᴀʀɢᴇ Aᴍᴍᴏᴜɴᴛ   : {actual_amount_rounded} Tᴋ\n"
                        f"➪ Fᴇᴇ (1.15%)   : {fee_rounded} Tᴋ\n"
                        f"➪ Yᴏᴜ Pᴀɪᴅ       : {total_amount_received} Tᴋ\n"
                        "▔▔▔▔▔▔▔▔▔▔▔▔"
                    )

                    # টেলিগ্রামে টেক্সট পাঠানো
